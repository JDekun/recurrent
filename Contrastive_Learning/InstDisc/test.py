...
def NN(epoch, net, lemniscate, trainloader, testloader, recompute_memory=0):
    ...
    if hasattr(trainloader.dataset, 'imgs'):
        ...
        trainLabels = torch.LongTensor(trainloader.dataset.data).cuda()
    ...
    if recompute_memory:
        ...
        trainLabels = torch.LongTensor(temploader.dataset.data).cuda()
...
def kNN(epoch, net, lemniscate, trainloader, testloader, K, sigma, recompute_memory=0):
    ...
    with torch.no_grad():
        ...
        for batch_idx, (inputs, targets, indexes) in loader:
            ...
            candidates = trainLabels.contiguous().view(1,-1).expand(batchSize, -1)
            retrieval = torch.gather(candidates, 1, yi)
